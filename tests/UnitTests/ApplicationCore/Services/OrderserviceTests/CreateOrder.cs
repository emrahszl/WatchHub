...
namespace UnitTests.ApplicationCore.Services.OrderserviceTests
{
    public class CreateOrder
    {
        ...
        [Fact]
        public async Task EmptyCartExceptionWhenCartIsEmpty()
        {
            ...
            var emptyBasket = new Basket() { Id = 1, BuyerId = _buyerId, BasketItems = new() };

            _mockBasketRepo.FirstOrDefaultAsync(Arg.Any<ISpecification<Basket>>()).Returns(emptyBasket);

            await Assert.ThrowsAsync<EmptyCartException>(() => orderService.CreateOrderAsync(_buyerId, _testAddress));
        }
    }
}
